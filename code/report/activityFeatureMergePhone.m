...
function activityFeatureMergePhone(pid,sid)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% load(['c:\dataProcessingFrameworkV2\data\activity\feature\field_' pid '_' sid '_act10_feature.mat'])
load(['c:\dataProcessingFrameworkV2\data\memphis\feature\field_' pid '_' sid '_act10_feature.mat'])

featureVal=[];
featureTs=[];
for i=1:length(F.window)
    if isfield(F.window(i).feature{5},'value')
        featureVal=[featureVal F.window(i).feature{5}.value{30}];
...
fid=fopen('C:\DataProcessingFrameworkV2\data\memphis\report\activityEpisodes\activityEpisodesPhoneAccel.csv','a');
line=[];
