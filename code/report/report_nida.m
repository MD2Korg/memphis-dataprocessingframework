...
pdaid=fopen([G.DIR.DATA G.DIR.SEP INFODIR G.DIR.SEP 'pdadata.csv']);
C=textscan(pdaid,'%s %s %s %s %s %s %s %s %s %s','delimiter',',');
fclose(pdaid);
for i=2:length(C{1})
    pid=C{1}{i};
    dates=C{3}{i};
    ...
    for d=1:length(data)
        if strcmp(data(d).pid,pid)~=1,continue;end;
        if strcmp(datestr(data(d).date,'mm/dd/yyyy'),datestr(dates,'mm/dd/yyyy'))~=1, continue;end
        found=1;
        if isfield(data(d),'drugtime')==0 || isempty(data(d).drugtime),
            data(d).drugstarttime=C{4}{i};data(d).drugendtime=C{5}{i};
            data(d).drugtype=C{7}{i};data(d).drugamount=C{8}{i};
            data(d).drugactual=C{9}{i};data(d).drugroute=C{10}{i};
        else
            data(d).drugstarttime=[data(d).drugstarttime ';' C{4}{i}];data(d).drugendtime=[data(d).drugendtime, ',' C{5}{i}];
            data(d).drugtype=[data(d).drugtype ';' C{7}{i}];data(d).drugamount=[data(d).drugamount ';' C{8}{i}];
            data(d).drugactual=[data(d).drugactual ';' C{9}{i}];data(d).drugroute=[data(d).drugroute ';' C{10}{i}];
        end
    end
    if found==0,
        data(end+1).pid=pid;
...
for i=2:length(C{1})
    ...
    for w=2:5
        ...
        for j=start_m:+1:end_m
            ...
            for d=1:length(data)
                if strcmp(data(d).pid,pid)~=1,continue;end;
                if strcmp(datestr(data(d).date,'mm/dd/yyyy'),datestr(j,'mm/dd/yyyy'))~=1, continue;end
                data(d).wid=wid;
                found=1;
            end
            if found==0,
