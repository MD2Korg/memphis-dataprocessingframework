function saveEMA2file_Memphis_format_v3(G,pid,sid,in)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fid=fopen('C:\DataProcessingFrameworkV2\data\memphis\report\Kedia\memphis_ema.csv','a');
...
for r=1:length(D.ema.data)     % # of data point
    ...
    [n s]=weekday(time);
    line=[line ',' num2str(D.ema.data(r).prompttimestamp) ',' time(1:19) ',' s ',' num2str(D.ema.data(r).context) ',' num2str(D.ema.data(r).delayduration) ','];
    %for c=14:2:col
    for c=1:length(D.ema.data(r).question)    %questions and corresponding answers
        ...
        elseif ~isempty(D.ema.data(r).question(c).response)
            ...
            %}
            line
            text=[text getAnswerChoices(D.ema.data(r).question(c).response)];
            line=[line text];
        else
...
function answerText=getAnswerChoices(choices)
    answerText='';
    if length(choices)==0
        return;
    end
...
%                 return
%             end
            answerText=[num2str(choices(i)) ' & ' answerText];
...
%         return
%     end
    answerText=[answerText num2str(choices(end))];
end
